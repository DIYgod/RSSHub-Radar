  ...
  "atcoder.jp": {
    ...
    },
    {
      ...
      source: ["/contests/archive",
        "/contests"],
      target: (params, url) =>
        `/atcoder/content/${new URL(url).searchParams.get('lang') ?? 'en'}/${new URL(url).searchParams.get('ratedType') ?? '0'}/${new URL(url).searchParams.get('category') ?? '0'}/${new URL(url).searchParams.get('keyword') ?? ''
        }`
    }]
  },
  "audiobar.cn": {
  ...
  linkresearcher: {
    ...
    ".": [{
      ...
      target: (_, url) => {
        ...
        const searchParams = new URL(url).searchParams;
        return `/linkresearcher/theses/${pathname.replace('/', '')}${searchParams.has('filters.subject') ? `&subject=${searchParams.get('filters.subject')}` : ''}${searchParams.has('filters.columns') ? `&columns=${searchParams.get('filters.columns')}` : ''
          }`;
      }
    }]
  },
